...
| Admin logs | `/` | `/admin/logs` | Verified | `375`, `430`, `768`, `1024`, `1440` | Mobile audit logs relied on a wide table with long unwrapped JSON/code previews, making log review awkward on small screens. | `src/app/(Private Pages)/admin/logs/page.tsx` | pending | pending | Added mobile log cards with wrapped, scrollable change details; kept a horizontally scrollable table for tablet/desktop; tightened mobile spacing. Verified no page-level horizontal overflow at all required widths. `npx tsc --noEmit` passed. |
