...
|---|---|---|---|---|---|---|---|---|---|
| Home | `/` | `/admin/home` | Verified | `375`, `430`, `768`, `1024`, `1440` | Public/admin home initially blocked by SSR `localStorage.getItem` crash; desktop nav enabled at `1024px` and caused 15px horizontal overflow. | `src/instrumentation.ts`, `src/components/providers/PostHogProvider.tsx`, `src/components/providers/PageTracker.tsx`, `src/components/header/Navbar.tsx` | pending | pending | Added server storage shim and browser-only PostHog loading so pages render; kept tablet landscape on mobile/tablet nav until `xl`; verified no page-level horizontal overflow on `/` and `/admin/home`; verified tablet menu opens at `1024px` without overflow. `npx tsc --noEmit` passed. `npm run lint` fails on pre-existing unrelated lint errors. |
| Admin dashboard | `/` | `/admin` | Not checked | - | - | - | - | - | Public paired with home; admin dashboard also checked separately because it is the admin landing page. |
| Admin pages index | `/` | `/admin/pages` | Not checked | - | - | - | - | - | Public paired with home; admin content index has no public equivalent. |
| Admin logs | `/` | `/admin/logs` | Not checked | - | - | - | - | - | Public paired with home; audit logs have no public equivalent. |
