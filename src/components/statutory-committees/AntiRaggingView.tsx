...
export function AntiRaggingHeroSection({
	data
}: {
	data: AntiRaggingData['hero'];
}) {
	const HeroIcon = getIconComponent(data.icon || 'Shield');
	return (
		<div
			className={`rounded-2xl p-12 text-white relative overflow-hidden bg-gradient-to-br ${
				data.gradient || 'from-red-600 via-orange-600 to-orange-500'
			} shadow-lg`}>
...
export function AntiRaggingDefinitionSection({
	data,
	heroIcon
}: {
	data: AntiRaggingData['definition'];
	heroIcon?: string;
}) {
	const HeroIcon = getIconComponent(heroIcon || 'Shield');
	return (
		<motion.div
...
export function AntiRaggingMembersSection({
	data
}: {
	data: AntiRaggingData['committeeMembers'];
}) {
	return (
		<motion.div
			initial={{ opacity: 0, y: 20 }}
...
export function AntiRaggingMeasuresSection({
	data
}: {
	data: AntiRaggingData['preventiveMeasures'];
}) {
	return (
		<motion.div
			initial={{ opacity: 0, y: 20 }}
...
export function AntiRaggingPunishmentsSection({
	data
}: {
	data: AntiRaggingData['punishments'];
}) {
	return (
		<motion.div
			initial={{ opacity: 0, y: 20 }}
...
export function AntiRaggingContactsSection({
	data
}: {
	data: AntiRaggingData['emergencyContacts'];
}) {
	return (
...
export default function AntiRaggingView({ data }: { data: AntiRaggingData }) {
	return (
		<div className='space-y-6 sm:space-y-8'>
			<AntiRaggingHeroSection data={data.hero} />
			<AntiRaggingDefinitionSection
				data={data.definition}
				heroIcon={data.hero.icon}
			/>
			<AntiRaggingMembersSection data={data.committeeMembers} />
			<AntiRaggingMeasuresSection data={data.preventiveMeasures} />
			<AntiRaggingPunishmentsSection data={data.punishments} />
			<AntiRaggingContactsSection data={data.emergencyContacts} />
		</div>
	);
}
