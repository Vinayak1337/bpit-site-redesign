...
import { Target, Users, Award, FileText, Settings } from 'lucide-react';

type Props = {
	data: IqacData;
};

export function IqacAboutSection({ data }: { data: IqacData['about'] }) {
	return (
		<motion.div
			...
			<div className='prose prose-gray max-w-none'>
				...
				<div className='bg-blue-50 border border-blue-200 rounded-lg p-4'>
					...
					<p className='text-blue-700 text-sm mb-3'>{data.vision}</p>
					<h4 className='font-semibold text-blue-800 mb-2'>Mission:</h4>
					<p className='text-blue-700 text-sm'>{data.mission}</p>
				</div>
			</div>
		</motion.div>
	);
...
export function IqacObjectivesSection({ data }: { data: IqacData['objectives'] }) {
	return (
		<motion.div
			initial={{ opacity: 0, y: 20 }}
			animate={{ opacity: 1, y: 0 }}
						...
						<motion.div
							...
							<div className='flex items-start gap-4'>
								<div className='p-3 bg-white rounded-lg'>
									<IconComponent className={`w-6 h-6 ${objective.iconColor}`} />
								</div>
								<div>
...
export function IqacFunctionsSection({ data }: { data: IqacData['functions'] }) {
	return (
		<motion.div
			initial={{ opacity: 0, y: 20 }}
			animate={{ opacity: 1, y: 0 }}
...
export function IqacMembersSection({ data }: { data: IqacData['committeeMembers'] }) {
	return (
		<motion.div
			initial={{ opacity: 0, y: 20 }}
			animate={{ opacity: 1, y: 0 }}
...
export function IqacInitiativesSection({ data }: { data: IqacData['initiatives'] }) {
	return (
		<motion.div
			initial={{ opacity: 0, y: 20 }}
			animate={{ opacity: 1, y: 0 }}
				...
				{data.map((initiative, index) => {
					...
					return (
						<motion.div
							...
							<div className='flex items-start gap-4'>
								<div className='p-3 bg-white rounded-lg'>
									<IconComponent className={`w-6 h-6 ${initiative.iconColor}`} />
								</div>
								<div>
									<h4 className='font-semibold text-gray-900 mb-2'>
										{initiative.title}
...
export function IqacAqarSection({ data }: { data: IqacData['aqar'] }) {
	return (
		<motion.div
			initial={{ opacity: 0, y: 20 }}
			animate={{ opacity: 1, y: 0 }}
...
export default function IqacView({ data }: Props) {
	return (
		<div className='space-y-6 sm:space-y-8'>
			<IqacAboutSection data={data.about} />
			<IqacObjectivesSection data={data.objectives} />
			<IqacFunctionsSection data={data.functions} />
			<IqacMembersSection data={data.committeeMembers} />
			<IqacInitiativesSection data={data.initiatives} />
			<IqacAqarSection data={data.aqar} />
		</div>
	);
}
