...
// Preview Component
const StatutoryOverviewPreview = ({ data }: { data: StatutoryOverviewData }) => {
	const { hero, committees } = data;

	return (
		<div className='space-y-6 sm:space-y-8 p-4'>
			{/* Hero Section */}
			<div className='bg-gradient-to-r from-green-50 to-blue-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 border border-green-200 text-center mb-6 sm:mb-8'>
...
export default function StatutoryOverviewEditor({ initialData, pageSlug }: Props) {
	...
	return (
		<Editable
			...
			<StatutoryOverviewPreview data={previewData} />
		</Editable>
	);
}
