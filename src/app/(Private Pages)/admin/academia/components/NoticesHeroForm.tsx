...
type Props = {
	...
	onChange?: (data: NoticesHeroData) => void;
};

const normalize = (v: Partial<FormValues>): NoticesHeroData => ({
	title: (v.title ?? '').trim(),
	subtitle: (v.subtitle ?? '').trim()
});

export default function NoticesHeroForm({ initialData, onChange }: Props) {
	...
	const handleSubmit = (values: FormValues) => {
		...
		startTransition(async () => {
			const r = await updateNoticesHero(payload);
		...
		<Form {...form}>
			<form onSubmit={form.handleSubmit(handleSubmit)} className='space-y-6'>
				<div className='flex items-center justify-between'>
					<h3 className='text-lg font-semibold text-gray-900'>Notices Hero</h3>
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Title and subtitle shown at the top of Notices & Circulars.
						</CardDescription>
					</CardHeader>
