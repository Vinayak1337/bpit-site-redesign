...
export default function AcademiaHeroForm({
	...
	const handleSubmit = (values: FormValues) => {
		...
		startTransition(async () => {
			const result = await updateAcademiaHero(pageSlug, payload);
			if (!result.ok) {
		...
		<Form {...form}>
			<form onSubmit={form.handleSubmit(handleSubmit)} className='space-y-6'>
				<div className='flex items-center justify-between'>
					<h3 className='text-lg font-semibold text-gray-900'>Academia Hero</h3>
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Update the main headline, description, gradient, and background image for the Academia section.
						</CardDescription>
					</CardHeader>
