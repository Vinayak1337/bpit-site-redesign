...
type Props = {
	...
	onChange?: (data: CalendarHeroData) => void;
};

const normalize = (v: Partial<FormValues>): CalendarHeroData => ({
	title: (v.title ?? '').trim(),
	subtitle: (v.subtitle ?? '').trim()
});

export default function CalendarHeroForm({ initialData, onChange }: Props) {
	...
	const handleSubmit = (values: FormValues) => {
		...
		startTransition(async () => {
			const r = await updateCalendarHero(payload);
			...
			<form onSubmit={form.handleSubmit(handleSubmit)} className='space-y-6'>
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Title and subtitle shown at the top of the Academic Calendar
							page.
						</CardDescription>
					</CardHeader>
