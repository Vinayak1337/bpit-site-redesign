...
export default function PlacementOverviewSection({ data }: PlacementOverviewSectionProps) {
	return (
			...
			<section className={`relative py-20 bg-gradient-to-r ${data.hero.gradient}`}>
				<div className="relative z-10 container mx-auto px-4">
					...
					>
						...
						>
							<div className={`p-4 ${data.hero.iconColor} backdrop-blur-sm rounded-2xl border border-white/20`}>
								{React.createElement(iconMap[data.hero.icon as keyof typeof iconMap] || Briefcase, { className: "w-12 h-12" })}
							</div>
						</motion.div>
						...
						<p className={`text-xl md:text-2xl ${data.hero.textColor} mb-8 leading-relaxed`}>
							{data.hero.subtitle}
						</p>
				...
				<div className="container mx-auto px-4">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
						{data.teamMembers.map((member, index) => (
							...
							>
								<div className="w-24 h-24 rounded-full mx-auto mb-6 flex items-center justify-center border-4 border-white shadow-lg bg-blue-500 text-white text-2xl font-bold">
									{member.initials}
								</div>
