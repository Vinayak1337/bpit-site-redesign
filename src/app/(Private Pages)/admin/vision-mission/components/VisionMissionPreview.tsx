...
const getColorClasses = (color: VisionMissionData['pillars'][number]['color']) => {
	const colorMap = {
		blue: {
			bg: 'bg-blue-100',
			icon: 'text-blue-600'
...
export default function VisionMissionPreview({ data }: Props) {
	return (
		<div className='space-y-6 sm:space-y-8 bg-gray-50 p-6 rounded-lg border max-h-[80vh] overflow-y-auto'>
			{/* Hero Section */}
			<motion.div
						...
						{data.pillars.map((pillar, index) => {
							...
							return (
								<motion.div
									...
									<div className={`w-10 h-10 sm:w-12 sm:h-12 ${colorClasses.bg} rounded-lg flex items-center justify-center mb-3 sm:mb-4`}>
										{React.createElement(getIcon(pillar.icon), {
											className: `w-5 h-5 sm:w-6 sm:h-6 ${colorClasses.icon}`
										})}
									</div>
				...
				<motion.div
					...
					<div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6'>
						{data.aspirations.map((aspiration, index) => {
							const colorClasses = getColorClasses(aspiration.color);
							return (
								<motion.div
									key={index}
									whileHover={{ scale: 1.02 }}
